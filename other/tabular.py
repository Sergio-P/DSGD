# Based on: https://github.com/yashu-seth/pytorch-tabular/blob/master/pytorch_tabular.py
...
class TabularDataset(Dataset):
    def __init__(self, data, cat_cols=None, output_col=None):
        ...
        if output_col:
            ynp = data[output_col].values
            self.k = int(data[output_col].nunique())
            self.y = torch.nn.functional.one_hot(torch.LongTensor(ynp), self.k).float()
        else:
...
class TabularNN(nn.Module):
    ...
    def forward(self, cont_data, cat_data, print_all=False):
