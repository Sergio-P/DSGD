...
dd = data.drop_duplicates(["age", "gender"]).sort_values(["gender", "age"])
print(dd)

dataset_test = TabularDataset(dd, data.columns.tolist(), "cls")
dataloader_test = DataLoader(dataset_test, BATCH_SIZE, shuffle=False, num_workers=1)
...
for y, cont_x, cat_x in dataloader_test:
    preds = model(cont_x, cat_x, print_all=True)[:, 1]
